...
const ICONS: Record<string, string> = {
  traversable: '◦', path: '→', source: 'S', destination: 'D',
...
const LAYER_INFO: Record<string, string> = {
  ...
  path:        'Directed path segments with explicit travel direction. Draw edges by dragging between adjacent cells.',
...
export default function LayerPanel() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 0 }}>
      ...
      {/* Cell info panel */}
      {selectedCell && !!selectedCell.assigned && (
        <CellInfoPanel
          key={selectedCellId}
          cell={selectedCell}
          pt={pt}
      ...
      <div style={{ marginBottom: 14, flexShrink: 0 }}>
        ...
        <StatRow label="Typed cells" value={map.cells.filter(c => !!c.assigned).length} pt={pt} />
